...
n_critic = 5        # Train the generator every n_critic iterations
...
# WGAN Optimizers
optimizer_G = torch.optim.RMSprop(itertools.chain(netG_A2B.parameters(), netG_B2A.parameters()), lr=lr)
optimizer_D_A = torch.optim.RMSprop(netD_A.parameters(), lr=lr)
optimizer_D_B = torch.optim.RMSprop(netD_B.parameters(), lr=lr)
...
for epoch in range(start_epoch, n_epochs):
    ...
    for i, batch in enumerate(dataloader):
        ...
        if i % n_critic == 0:
            ...
            # WGAN LOSS
            loss_GAN_A2B_WGAN = -torch.mean(pred_fake)

            fake_A = netG_B2A(real_B)
            pred_fake = netD_A(fake_A)
            # loss_GAN_B2A = criterion_GAN(pred_fake, target_real)
            # WGAN LOSS
            loss_GAN_B2A_WGAN = -torch.mean(pred_fake)
        else:
            loss_GAN_A2B_WGAN = 0
            loss_GAN_B2A_WGAN = 0
        ...
        # WGAN LOSS
        loss_G += (loss_GAN_A2B_WGAN + loss_GAN_B2A_WGAN) * 1.0
        ...
        # WGAN netD_A
        loss_D_A_WGAN = -torch.mean(pred_real) + torch.mean(pred_fake)
        loss_D_A_WGAN.backward()
        ...
        # WGAN netD_B
        loss_D_B_WGAN = -torch.mean(pred_real) + torch.mean(pred_fake)
        loss_D_B_WGAN.backward()
        ...
        if iter % log_interval == 0:
            ...
            print('GAN loss:', as_np(loss_GAN_A2B_WGAN), as_np(loss_GAN_B2A_WGAN))
            ...
            print('D loss:', as_np(loss_D_A_WGAN), as_np(loss_D_B_WGAN))
            print('time:', time.time() - prev_time)
            prev_time = time.time()

        # Print outputs
        if iter % image_save_interval == 0:
            output_path_ = os.path.join(output_imgs_path, str(iter + load_iter*1000)+'WGAN')
        ...
        if iter % model_save_interval == 0:
            torch.save(netG_A2B.state_dict(), os.path.join(output_weights_path, 'WGAN_netG_A2B_i{}.pth'.format(iter + load_iter*100)))
            torch.save(netG_B2A.state_dict(), os.path.join(output_weights_path, 'WGAN_netG_B2A_i{}.pth'.format(iter+ load_iter*100)))
            torch.save(netD_A.state_dict(), os.path.join(output_weights_path, 'WGAN_netD_A_i{}.pth'.format(iter + load_iter*100)))
            torch.save(netD_B.state_dict(), os.path.join(output_weights_path, 'WGAN_netD_B_i{}.pth'.format(iter + load_iter*100)))
